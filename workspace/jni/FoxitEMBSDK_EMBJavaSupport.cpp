...
(JNIEnv *pEnv, jclass cls, jint textpage, jstring findwhat, jlong flag, jint startindex)
{
	...
	FS_WCHAR* buff = new FS_WCHAR[length+1];
	memcpy(buff, wcFind,length*sizeof(FS_WCHAR));
	buff[length] = '\0';
	FS_RESULT ret = FPDF_Text_FindStart(pTextPage,(FS_WCHAR*)buff, flag, startindex, &searchHandle);
	delete []buff;
    buff = NULL;
