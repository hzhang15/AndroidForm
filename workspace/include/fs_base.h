...
#ifndef _FS_DEF_BSTR_
#define _FS_DEF_BSTR_
/** @brief A byte string structure. Available since version 2.1.*/
typedef struct _FS_BSTR_
{
	/** @brief The length of the byte string buffer. */
	FS_INT32	len;
	/** @brief The byte string buffer. */
	FS_LPSTR	str;
}FS_BSTR;
#endif

#ifndef _FS_DEF_WSTR_
#define _FS_DEF_WSTR_
/**  @brief A 16-bit Unicode string structure. Available since version 2.1. */
typedef struct _FS_WSTR_
{
	/** @brief The length of the 16-bit Unicode string buffer. */
	FS_INT32	len;
	/** @brief The 16-bit Unicode string buffer. */
	FS_LPWSTR	str;
}FS_WSTR;
#endif
...
/** @brief Render context handle type. Available since version 2.1.*/
FS_DEFINEHANDLE(FS_RENDERCONTEXT);

/**@}*/

#ifndef _FSSYSTEMTIME_
#define _FSSYSTEMTIME_
/**
 * @brief A system time structure. Available since version 2.1.
 */
typedef struct _FS_SYSTEMTIME_ 
{
    FS_DWORD		wYear;			/**< @brief Year, a four-digit number, such as 2013. */
    FS_DWORD		wMonth;			/**< @brief Month, from 1 to 12. */
    FS_DWORD		wDayOfWeek;		/**< @brief Day of week, from 0 to 6. Omit this field if not used. */
    FS_DWORD		wDay;			/**< @brief Day of month, from 1 to 31. Omit this field if not used. */
    FS_DWORD		wHour;			/**< @brief Hour, from 0 to 23. */
    FS_DWORD		wMinute;		/**< @brief Minute, from 0 to 59. */
    FS_DWORD		wSecond;		/**< @brief Second, from 0 to 60. 60 for leap second. */
    FS_DWORD		wMilliseconds;	/**< @brief Millisecond, from 0 to 999. */
}FS_SYSTEMTIME;
#endif
...
#ifndef _FS_DEF_FILEREADWRITE_
#define _FS_DEF_FILEREADWRITE_
/** 
 * @brief Structure for reading and writing a file. Available since version 2.1.
 * @note This structure can be considered as a combination of ::FS_FILEREAD and ::FS_FILEWRITE.
 */
typedef struct  _FS_FILEREADWRITE_
{
	/** @brief The size of the data structure. It must be set to <I>sizeof</I>(::FS_FILEREADWRITE). */
	FS_DWORD	lStructSize;
	...
	*
	* @param[in] clientData		The user-supplied data.
	* @param[out] buffer		Pointer to a buffer used to receive the read data
	* @param[in] offset			Byte offset for the block, from the beginning of the file.
	* @param[in] size			The number of bytes for the block.
	*
	* @return	::FS_ERR_SUCCESS means success.<br>
	...
	*/
	...
	/**
	 * @brief	A callback function used to write a data block to the file.
	 *
	 * @param[in] clientData		The user-supplied data.
	 * @param[in] buffer			Pointer to a buffer containing the data which is to be written into the file.
	 * @param[in] offset			The offset in byte for the block, from the beginning of the file.
	 * @param[in] size				The count of bytes of the block.
	 *
	 * @return	::FS_ERR_SUCCESS means success.<br>
	...
	/**
	 * @brief	A callback function used to flush a stream.
	 *
	 * @param[in] clientData		The user-supplied data.
	 *
	 * @return	::FS_ERR_SUCCESS means success.<br>
...
#define FS_ERR_INVALIDTYPE		12
/** @brief Unsupported feature. */
#define FS_ERR_UNSUPPORTED		13
/** @brief Unrecoverable error. */
#define FS_ERR_UNRECOVERABLE	14
/** @brief Security handler error. */
#define FS_ERR_SECURITYHANDLER	15
/** 
 ...
 *			the DIB to white background by yourself.
 *
 ...
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note The format <b>FS_DIBFORMAT_RGB565</b> is not available for the official released package, if you would like to use this format, please contact us for a special build of the SDK package.
 *		 Also if <b>FS_DIBFORMAT_RGB565</b> is enabled, then the other formats would not be available anymore. 
 */
...
/**
 * @brief Help function to initialize a ::FS_BSTR structure.
 *
 * @param[in] bstr	Pointer to a ::FS_BSTR structure.
 *
 * @return None
 *
 * @note Available since version 2.1.
 */
void FS_BStr_Init(FS_BSTR* bstr);

/**
 * @brief Help funtion to release a ::FS_BSTR structure.
 *
 * @param[in] bstr	Pointer to a ::FS_BSTR structure.
 *
 * @return None
 ...
 * @note Available since version 2.1.
 */
void FS_BStr_Release(FS_BSTR* bstr);


/**
 * @brief Help function to initializing the size of a ::FS_BSTR structure.
 *
 * @param[in] bstr	Pointer to a ::FS_BSTR structure.
 * @param[in] size	The size used to initialize the ::FS_BSTR structure.
 *
 * @return None
 *
 * @note Available since version 2.1.
 */
void FS_BStr_SetSize(FS_BSTR* bstr, FS_INT32 size);

/**
 * @brief Help function to initializing a ::FS_WSTR structure.
 *
 * @param[in] wstr	Pointer to a ::FS_WSTR structure.
 *
 * @return None
 *
 * @note Available since version 2.1.
 */
void FS_WStr_Init(FS_WSTR* wstr);

/**
 * @brief Help function to release a ::FS_WSTR structure.
 *
 * @param[in] wstr	Pointer to a ::FS_WSTR structure.
 *
 * @return None
 ...
 * @note Available since version 2.1.
 */
void FS_WStr_Release(FS_WSTR* wstr);

/**
 * @brief Help function to initializing the size of a ::FS_WSTR structure.
 *
 * @param[in] wstr	Pointer to a ::FS_WSTR structure.
 * @param[in] size	The size used to initialize the ::FS_WSTR structure.
 *
 * @return None
 *
 * @note Available since version 2.1.
 */
void FS_WStr_SetSize(FS_WSTR* wstr, FS_INT32 size);

/**
 * @brief	Determine whether one rectangle is adjacent to another one in horizontal direction or vertical direction.
 *
 * @param[in] rect1					First rectangle.
 * @param[in] rect2					Second rectangle.
 * @param[in] alignmentTolerance	Tolerance of alignment to be allowed between to rectangles.
 * @param[in] distanceTolerance		Tolerance of distance to be allowed between to rectangles.
 * @param[in] direction				Used to specify the direction, in which to judge the adjacent status between two rectangles:<br>
 *									<ul>
 *									<li>0: horizontal direction.</li>
 *									<li>1: vertical direction.</li>
 *									</ul>
 * @param[out] adjacent				Used to receive the status as result.<br>
 *									<b>TRUE</b> means parameter <i>rect1</i> is adjacent to parameter <i>rect2</i>, while <b>FALSE</b> means not.
 *
 * @return	::FS_ERR_SUCCESS means success.<br>
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note Available since version 2.1.
 */
FS_RESULT	FS_Rect_IsAdjacent(FS_LPCRECTF rect1, FS_LPCRECTF rect2, FS_FLOAT alignmentTolerance, FS_FLOAT distanceTolerance, FS_DWORD direction, FS_BOOL* adjacent);
