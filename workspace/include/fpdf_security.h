...
/**
 * @brief Check if the document is encrypted in standard way.
 *
 * @param[in]		document			A handle to the PDF document.
 * @param[out]		isStdEncrypted		Used to received a boolean value: <b>TRUE</b> means document is encrypted in standard way, otherwise <b>FALSE</b>.
 *
 * @return	::FS_ERR_SUCCESS means success.<br>
 *			 For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 ...
 */
FS_RESULT FPDF_Doc_IsStdEncrypted(FPDF_DOCUMENT document, FS_BOOL* isStdEncrypted); 

/**
 * @brief Check if the user is the owner of the document.
 *
 * @param[in]		document		A handle to the pdf document.
 * @param[out]		isOwner			Used to receive a boolean value. <b>TRUE</b> means user is the owner of the PDF document, otherwise <b>FALSE</b>.
 *
 * @return	::FS_ERR_SUCCESS means success.<br>
 *			 For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note Available since version 2.1.
 */
FS_RESULT FPDF_Doc_IsOwner(FPDF_DOCUMENT document, FS_BOOL* isOwner); 

/**
 * @brief Start to encrypt the PDF document in standard way.
 *
 * @details	If this function ::FPDF_Security_StartStdEncrypt returns ::FS_ERR_TOBECONTINUED, 
 *			::FPDF_Security_ContinueStdEncrypt should be called to continue doing standard encryption.
 *
 * @param[in]		document			A handle to the PDF document.
 * @param[in]		permissions			Permission value. See macro definitions <b>FPDF_PERMISSION_XXX</b>.
 * @param[in]		userPassword		The user password,and it could be set to <b>NULL</b>.
 * @param[in]		ownerPassword		The owner password,and it could be set to <b>NULL</b>.
 * @param[in]		cipher				The type of encryption algorithm. See macro definitions <b>FPDF_CIPHER_XXX</b>.
 * @param[in]		key_bytes			The length of key for AES or RC4 algorithm, and it should be 16 or 32.<br>
 *										Specially, for RC4 algorithm, only 16 is supported.
 * @param[in]		bEncryptMetadata	Used to specify whether to encrypt the metadata or not.
 * @param[in]		fwEncryptedFile		A pointer to a ::FS_FILEWRITE structure. 
 *
 * @return	::FS_ERR_TOBECONTINUED means to be continued.<br>
 *			 For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note Available since version 2.1.
 */
FS_RESULT FPDF_Security_StartStdEncrypt(FPDF_DOCUMENT document, FS_DWORD permissions,const FS_BSTR* userPassword,const FS_BSTR* ownerPassword, 
										FS_INT32 cipher, FS_INT32 key_bytes,FS_BOOL bEncryptMetadata,FS_FILEWRITE* fwEncryptedFile);

/**
 * @brief Continue to encrypt the PDF document in standard way.
 *
 * @details	If function ::FPDF_Security_StartStdEncrypt or ::FPDF_Security_ContinueStdEncrypt returns ::FS_ERR_TOBECONTINUED, 
 *			this function should be called continuously until ::FPDF_Security_ContinueStdEncrypt returns ::FS_ERR_SUCCESS.
 *
 * @param[in]		document			A handle to the PDF document.
 * @param[in]		pause				A FS_PAUSE handle, could be set to NULL if don't need to pause.
 *
 * @return	::FS_ERR_SUCCESS means success.<br>
 *			::FS_ERR_TOBECONTINUED means the security progress needs to be continued.<br>
 *			 For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note Available since version 2.1.
 */
FS_RESULT FPDF_Security_ContinueStdEncrypt(FPDF_DOCUMENT document, FS_PAUSE* pause);
