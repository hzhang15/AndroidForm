 ...
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 * @note This function is deprecated, instead of this, "FPDF_Text_GetOriginF" is recommened.
 */
FS_RESULT FPDF_Text_GetOrigin(FPDF_TEXTPAGE textPage, FS_INT32 index, FS_POINT* origin);

/**
 * @brief Get the origin position (float) of a particular character. 
 *		
 * @details The X and Y positions are measured in the PDF "user space".
 *
 * @param[in]  textPage			A text page handle returned by ::FPDF_Text_LoadPage.
 * @param[in]  index			A zero-based index of the character.
 * @param[out] origin			A pointer to an ::FS_POINTF structure used to receive the X and Y position 
 *								of the character origin.
 *
 * @return	::FS_ERR_SUCCESS means get the origin position successfully.<br>
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 ...
 */
FS_RESULT FPDF_Text_GetOriginF(FPDF_TEXTPAGE textPage, FS_INT32 index, FS_POINTF* origin);
 ...
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note This function is deprecated, instead of this, "FPDF_Text_GetCharBoxF" is recommened.
 */
FS_RESULT FPDF_Text_GetCharBox(FPDF_TEXTPAGE textPage, FS_INT32 index, FS_RECT* rect);

/**
 * @brief Get the bounding box (float) of a particular character.
 *
 * @details All positions are measured in the PDF "user space".
 *
 * @param[in]  textPage			A text page handle returned by ::FPDF_Text_LoadPage.
 * @param[in]  index			A zero-based index of the character.
 * @param[out] rect				A pointer to an ::FS_RECTF used to receive the the 4 positions (left, right, 
 *								bottom, top)of the character box.
 *
 * @return	::FS_ERR_SUCCESS means get the origin position successfully.<br>
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 ...
 * @note Available since version 2.1.
 */
FS_RESULT FPDF_Text_GetCharBoxF(FPDF_TEXTPAGE textPage, FS_INT32 index, FS_RECTF* rect);
 ...
 *
 * @param[in]  textPage			A text page handle returned by ::FPDF_Text_LoadPage.
 * @param[in]  rectIndex		A zero-based index the rectangle to be got.
 ...
 */
FS_RESULT FPDF_Text_GetRect(FPDF_TEXTPAGE textPage, FS_INT32 rectIndex, FS_RECTF* rect);

/** 
 * @brief Get the direction of a rectangular area from the result generated by ::FPDF_Text_CountRects.
 *
 * @param[in]  textPage			A text page handle returned by ::FPDF_Text_LoadPage.
 * @param[in]  rectIndex		A zero-based index of the rectangle to be got.
 * @param[out] direction		The trend for the text on the specific page rect:
 *								<ul>
 *								<li>0: From left to right.</li>
 *								<li>1: From right to left.</li>
 *								<li>2: From bottom to top.</li>
 *								<li>3: From top to bottom.</li>
 *								<li>-1: Unknown direction.</li>
 *								</ul>
 *
 * @return	::FS_ERR_SUCCESS means get the rectangle successfully.  <br>
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 ...
 * @note Available since version 2.1.
 */
FS_RESULT FPDF_Text_GetDirection(FPDF_TEXTPAGE textPage, FS_INT32 rectIndex, FS_INT32* direction);
 ...
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note This function is deprecated, instead of this, "FPDF_Text_GetBoundedTextF" is recommended.
 */
FS_RESULT FPDF_Text_GetBoundedText(FPDF_TEXTPAGE textPage, FS_RECT rect, 
								   FS_LPWSTR buffer, FS_LPDWORD buflen);

/** 
 * @brief Extract a Unicode text within a rectangular boundary(float) on the page.
 *
 * @details	Usually, this function needs to be called twice:<br>
 *			For first time, set parameter <i>buffer</i> to <b>NULL</b>, 
 *			and parameter <i>buflen</i> will receive the actual count of characters to be got when function returns ::FS_ERR_SUCCESS.<br>
 *			Then application should allocate a buffer with parameter <i>buflen</i>.
 *			For second time, input valid <i>buffer</i> with enough size and its count <i>buflen</i> to get the Unicode text.
 *
 * @param[in]		textPage	A text page handle returned by ::FPDF_Text_LoadPage.
 * @param[in]		rect		A ::FS_RECTF structure used to specify the 4 boundaries(left, top, right, bottom) of the rectangle.
 * @param[out]		buffer		A Unicode buffer (allocated by the application) used to receive the text.
 * @param[in,out]	buflen		When parameter <i>buffer</i> is <b>NULL</b>, this will received the actual count of charaters(not bytes, including terminator) as a result when returns ::FS_ERR_SUCCESS.<br>
 *								When parameter <i>buffer</i> is valid with enough size, this is the size(not bytes) of <i>buffer</i> as an input value.
 *
 * @return	::FS_ERR_SUCCESS means get the bounded text successfully or just get the actual size of the text string.<br>
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note Available since version 2.1.
 */
FS_RESULT FPDF_Text_GetBoundedTextF(FPDF_TEXTPAGE textPage, FS_RECTF rect, 
									FS_LPWSTR buffer, FS_LPDWORD buflen);
/** 
 ...
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note This function is deprecated, instead of this, "FPDF_Text_CountBoundedSegmentsF" is recommened.
 */
FS_RESULT FPDF_Text_CountBoundedSegments(FPDF_TEXTPAGE textPage, FS_RECT rect, FS_INT32* count);

/** 
 * @brief Get the count of text segments within a rectangular boundary(float) on the page. 
 *
 * @param[in]  textPage			A text page handle returned by ::FPDF_Text_LoadPage.
 * @param[in]  rect				A specified rectangle, in which the count of segments will be got. 
 * @param[out] count			Used to receive the count of segments.
 *
 * @return	::FS_ERR_SUCCESS means count segments successfully.<br>
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note Available since version 2.1.
 */
FS_RESULT FPDF_Text_CountBoundedSegmentsF(FPDF_TEXTPAGE textPage, FS_RECTF rect, FS_INT32* count);
