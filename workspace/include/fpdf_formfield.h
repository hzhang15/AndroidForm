...
typedef struct _FPDF_FORMFILL_JSPLATFORM_
{
	 ...
	 *
	 * @param[in] clientData	The user-supplied data.
	 * @param[in] mailData		A pointer to the data buffer to be sent as an attachment. It can be <b>NULL</b>.
	 ...
	 *		the current document will be mailed as an attachment to all recipients, if mailData is not NULL and
	 *		length is not 0, then the data which "mailData" points will be sent as a mail attachment.
	 */
...
/**
 * @brief Get quadding (justification) of the form field to be used in displaying the text.
 *
 * @param[in]  formfield			The handle to the form field.
 * @param[out] justification		Used to receive quadding (justification) of form field. See macro definitions <b>FPDF_JUSTIFICATION_XXX</b>.
 * 
 * @return	::FS_ERR_SUCCESS means success.<br>
 *			For more definitions please see macro definitions <b>FS_ERR_XXX</b>.
 *
 ...
 */
FS_RESULT FPDF_FormField_GetJustification(FPDF_FORMFIELD formfield,  FS_INT32* justification);
