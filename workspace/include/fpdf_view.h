 ...
 *							</ul>
 * @param[out] matrix		Used to receive page transformation matrix.
 *
 ...
 */
FS_RESULT FPDF_Page_GetMatrix(FPDF_PAGE page, 
 ...
 * 
 * @param[in]	page		Handle to a <b>FS_PAGE</b> object which is a PDF page object. It should be valid.
 * @param[out]	rotation	Pointer to a ::FS_INT32 object that receives page rotation state. See macro definitions <b>FPDF_PAGEROTATION_XXX</b>.
 *
 * @return 	::FS_ERR_SUCCESS if succeeds.<br>
 *			::FS_ERR_PARAM if parameter <i>page</i> or <i>rotation</i> is a <b>NULL</b> pointer.<br>
 * 			::FS_ERR_INVALIDTYPE if parameter <i>page</i> is not a type of PDF page.<br>
 *			For more error codes, please refer to macro definitions <b>FS_ERR_XXX</b>.
 *
 * @note Available since version 2.1.
 */
FS_RESULT	FPDF_Page_GetRotation(FPDF_PAGE page, FS_INT32* rotation);
 ...
 *			much memory consumption (because no page objects will be cached).
 *
 * @param[in] rContext		A platform related device context.<br>
 *							For example, on Windows platform it's a HDC handler; on IOS platform it's a CGContext reference.
 * @param[in] page			A page handle. The page has to be parsed first.
 * @param[in] start_x		The left pixel position of the display area in the device coordination
 * @param[in] start_y		The top pixel position of the display area in the device coordination
 * @param[in] size_x		The horizontal size (in pixels) for displaying the page
 ...
 *							<li>3: rotated 90 degrees counter-clockwise</li>
 *							</ul>
 * @param[in] flags		0 means normal display. This can also be the one or combination of macro definitions <b>FPDF_RENDER_XXX</b>.
 * @param[in] clip		A ::FS_RECT structure of a clip rectangle (in DIB device coordinations),
 *						or NULL if no clipping is needed.
 * @param[in] pause		A ::FS_PAUSE structure that can pause the rendering process.
 *						This can be <b>NULL</b> if no pausing is needed.
 ...
 *			The render flag ::FPDF_RENDER_NOWIDGET should be set if method ::FPDF_FormFill_Draw is called after,
 *			because ::FPDF_FormFill_Draw takes care of "rendering widgets" in the FormFill environment.
 */
FS_RESULT FPDF_RenderPage_StartOnContext(FS_RENDERCONTEXT rContext, FPDF_PAGE page,
										 FS_INT32 start_x, FS_INT32 start_y, FS_INT32 size_x, FS_INT32 size_y, FS_INT32 rotate, FS_INT32 flags,
										 FS_RECT* clip, FS_PAUSE* pause);
